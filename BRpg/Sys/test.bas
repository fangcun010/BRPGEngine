...
Const True=1'yes
Const False=0'no
Const Picres_Pc="Jh.Rlb"'pc图片资源
Const Picres="Jh.Lib"'小机图片资源
Const Respath="异域传说\"
Const Picsay_Id=27
...
JP0:
	loadmap("方寸居",2)
	ggactorxy(8,6)
	Createnpc(15,8,5,Face_down,0,NPC_Round)'创建NPC
	showactxymap()
	Say(20,"你好！我是方寸。")
	Say(20,"这是对话的例子")
	Say(20,"这是消息的例子")
	...
	Say(20,"这是获取物品的例子")
	showactxymap()
	delallnpc(0)'删除所有NPC除了主角
JP1:
	Showload()
	g_jp=1
	loadmap("方寸居",2)
	Setmap1(7,13,0,1)'设置行走事件
	Setmap1(11,2,0,2)
	Createnpc(15,8,5,Face_down,0,NPC_Round)'创建NPC
JP1GO:
	...
	if g_message=1 then
		...
		Say(20,"走到房门哪试试看。")
		goto jp1go
	else if g_message=-1 then
		Say(20,"行走事件示例")
		Say(20,"按退出键进入物品菜单使用木剑看看。")
		goto jp1go
	else if g_message=-2 then
		Say(20,"战斗测试！")
		lin=gamefight("00","001",5,5,0)
		Say(20,"测试结束！")
		goto jp1go
	end if
End Sub
...
Sub Newgamedh()
	...
	Printstr(Text$,(Lcd_Wid-Len(Text$)*8)/2,(Lcd_Hgt-16)/2,1,0)
	Text$="去"
	Printstr(Text$,(Lcd_Wid-Len(Text$)*8)/2,((Lcd_Hgt-16)/2)+16,1,0)
	Forwaitkey()
	Fillpage(-1,0,0,240,160,0)
	Text$="那好，你跟我来。。。。"
	Printstr(Text$,(Lcd_Wid-Len(Text$)*8)/2,(Lcd_Hgt-16)/2,1,0)
...
Ceshi:
	...
	Data 32,0,32,0,32,0,32,0,32,0
	Data 32,0,32,0,32,0,32,0,32,0
	Data 32,81,32,0,32,0,32,0,32,0
	Data 32,0,32,0,32,0,32,0,32,0
	Data -32,96,32,0,32,0,32,0,32,0
	Data 32,0,32,0,32,0,32,0,32,0
	Data 32,0,32,0,32,0,32,0,32,0
	Data 32,0,32,0,32,0,32,0,32,0
	Data 32,0,32,0,32,0,32,0,32,0
